...
// Node's global fetch (undici) defaults to 300s headers/body timeouts, which resets
// slow thinking-mode generations before REQUEST_TIMEOUT_MS can ever apply (observed:
// batch attempts dying at exactly 5 x 300s with "fetch failed"). Lift undici's socket
// timeouts slightly above the AbortSignal so the explicit 10-minute guard governs.
// NOTE: the dispatcher must come from the same undici copy as the fetch call — Node's
// global fetch uses an internal undici build and rejects this package's Agent, so
// callKimi uses undici's own fetch with this matching Agent.
const KIMI_FETCH_DISPATCHER = new Agent({
...
// End-to-end mutation configuration: first enabled account alphabetically under the MCC
// (2132527958): 10X AUTO GROUP INC (8847499121). Candidates pre-generated by
// scripts/prepare-mutation-e2e.ts (last 30 days, aligned campaign-scope aggregation).
const DEFAULT_CUSTOMER = "8847499121";
const DEFAULT_CANDIDATES_SOURCE = "runs/mutation-e2e-8847499121/organizations/8847499121/candidates.json";
...
async function main(): Promise<void> {
  ...
  const run: KimiRunOptions = {
    customerId: optionValue(args, "--customer") ?? DEFAULT_CUSTOMER,
    label: optionValue(args, "--label") ?? "10x-auto",
  ...
  const manifestBase = {
    ...
    startedAt,
    purpose: `Kimi (${kimi.model}) 30-day classification run with optional guarded negative-keyword mutation`,
    requestedDateRange: { from: dateRange.startDate, to: dateRange.endDate },
    candidateSource: `${run.candidatesSource} (pre-generated input file; no Google Ads refetch)`,
    resumedFrom: optionValue(args, "--resume-from"),
  ...
  const resumableOutputs = resumeFrom
    ? await loadResumableBatchOutputs(workspace, resumeFrom, organization.customerId, batches)
    : new Map<string, ResumableBatchOutput>();
  ...
  await Promise.allSettled(batches.map((batch, index) => llmLimit(async () => {
    ...
    // Resume: reuse the prior run's validated output for this exact batch content
    // instead of re-calling the API. Only batches with identical candidate itemIds
    // are reused (verified in loadResumableBatchOutputs).
    const resumed = resumableOutputs.get(batchId);
    if (resumed) {
      for (const attempt of resumed.attempts) telemetry.recordGeneration(attempt.usage);
      telemetry.recordBatch(true);
      organizationUsage = mergeUsage(organizationUsage, resumed.tokenUsage, resumed.attempts.length);
      batchTokenUsage.push({ batchId, status: "VALIDATED", candidateCount: batch.length, generationRequests: resumed.attempts.length, ...resumed.tokenUsage, reusedFrom: resumed.sourceRun });
      decisions.push(...resumed.decisions);
      if (resumed.rateLimitHeaders) rateLimitObservations.push({ batchId, ...resumed.rateLimitHeaders });
      await artifacts.write(`${basePath}/llm/batch-${batchId}-output.json`, { ...resumed.raw, reusedFrom: resumed.sourceRun });
      return;
    }
    try {
  ...
  logger.info({
    ...
    reusedBatches: batchTokenUsage.filter((batch) => batch.reusedFrom).length,
    tokenUsage: snapshot.tokenUsage,
...
async function callKimi(kimi: KimiEnv, request: Record<string, unknown>): Promise<{
  ...
  for (let attempt = 0; attempt <= 4; attempt += 1) {
    ...
    try {
      // undici's own fetch: Node's global fetch binds to Node's internal undici copy
      // and fails immediately when given this package's Agent as dispatcher.
      response = await undiciFetch(url, {
        ...
        dispatcher: KIMI_FETCH_DISPATCHER
...
interface ResumableBatchOutput {
...
// Loads validated batch outputs from a previous run for --resume-from. A batch is
// only reusable when its recorded decisions cover exactly the same candidate itemIds
// as this run's batch at the same index — guards against a changed candidates file,
// batch size, or --limit producing a shifted batch layout.
async function loadResumableBatchOutputs(
  workspace: string,
  resumeFrom: string,
  customerId: string,
  batches: ClassificationCandidate[][]
): Promise<Map<string, ResumableBatchOutput>> {
  const llmDir = resolve(workspace, resumeFrom, "organizations", customerId, "llm");
  let files: string[];
  try {
    files = await readdir(llmDir);
  } catch {
    throw new Error(`--resume-from run has no LLM artifacts for customer ${customerId}: ${llmDir}`);
  }
  const resumable = new Map<string, ResumableBatchOutput>();
  for (const file of files) {
    const match = /^batch-(\d{4})-output\.json$/u.exec(file);
    if (!match) continue;
    const batchId = match[1];
    if (batchId === undefined) continue;
    let parsed: Record<string, any>;
    try {
      parsed = JSON.parse(await readFile(resolve(llmDir, file), "utf8"));
    } catch { continue; }
    if (parsed?.status !== "VALIDATED") continue;
    if (!Array.isArray(parsed.decisions) || !isRecord(parsed.tokenUsage) || !Array.isArray(parsed.attempts)) continue;
    const batch = batches[Number(batchId) - 1];
    if (!batch) continue;
    if (parsed.decisions.length !== batch.length) continue;
    const wanted = new Set(batch.map((candidate) => candidate.itemId));
    if (!parsed.decisions.every((decision: ClassificationDecision) => wanted.has(decision.itemId))) continue;
    resumable.set(batchId, {
      decisions: parsed.decisions,
      tokenUsage: normalizeKimiUsage(parsed.tokenUsage),
      attempts: parsed.attempts,
      rateLimitHeaders: isRecord(parsed.rateLimitHeaders) ? parsed.rateLimitHeaders : null,
      raw: parsed,
      sourceRun: resumeFrom
    });
  }
  return resumable;
}
