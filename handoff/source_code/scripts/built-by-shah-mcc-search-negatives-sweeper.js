/**
 * BUILT BY SHAH — MCC Search Negatives Sweeper
 * Version: 1.1.0
 *
 * Sibling automation to the Hub-and-Spoke Engine. This script reviews Search
 * queries, adds the full query as an exact campaign negative when it passes all
 * safeguards, and records actionable results on that shop's Negatives Audit tab.
 *
 * INSTALL
...
function main() {
  ...
  if (!dueRows.length) {
    Logger.log('No eligible Search negatives accounts are due.');
    return;
  }
  ...
  Logger.log(
    'Starting Search negatives sweep ' + runId + ' for ' +
    ...
    for (i = 0; i < candidates.length; i++) {
      ...
      if (!campaign) {
        ...
        pushActionRow_(
          ...
          ruleResult.eligibleRules,
          'FAILED',
          'The Search campaign could not be resolved by ID.',
          'FAILED',
          timeZone,
          currency
        );
...
function selectHubAccounts_(hub, sheet) {
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    var lastRun = calendarDateText_(
      row[map['Negatives Last Successful Run']],
      timeZone
    );
...
function stampSuccessfulHubRows_(outputs) {
  ...
  var headers = sheet
    ...
    .getDisplayValues()[0];
  var map = buildHeaderMap_(headers);
  requireHeaders_(map, ['Account ID', 'Negatives Last Successful Run']);
  var timeZone = getHubTimeZone_(hub);
  ...
  for (var i = 0; i < outputs.length; i++) {
    ...
    sheet.getRange(
      rowNumber,
      map['Negatives Last Successful Run'] + 1
    ).setValue(dateText);
  }
}
...
function processRemoveRequests_(
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    if (channel && channel !== CONFIG.CHANNEL) continue;
...
function aggregateSearchTerms_(rows, actionStart, actionEnd) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    var searchTerm = String(row.searchTermView.searchTerm || '').trim();
