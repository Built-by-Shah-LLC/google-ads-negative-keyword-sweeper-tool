...
test("loads the authoritative Markdown rule set with unique rule IDs", async () => {
  ...
  assert.match(rules.markdown, /body shop auto repair near me/iu);
  assert.match(rules.markdown, /auto repair body shop/iu);
  assert.match(rules.markdown, /best toyota body shop near me/iu);
  ...
  assert.match(rules.markdown, /auto body manager/iu);
  assert.match(rules.markdown, /apprenticeship auto body repair/iu);
  assert.ok(rules.ruleIds.includes("POL-CAREERS-NEGATIVE"));
